...
app = DjangoDash('MaxT', external_stylesheets=external_stylesheets)
app.layout = html.Div([
    html.H5('MaxT Slider Graph'),
...
MinT = graph_data['MinT'].to_numpy()
WindSpeed = graph_data['WindSpeed'].to_numpy()
app = DjangoDash('MinT', external_stylesheets=external_stylesheets)
app.layout = html.Div([
    html.H5('MinT Slider Graph'),
    dcc.Graph(id='slider-graph', animate=True, style={"backgroundColor": "#1a2d46", 'color': '#ffffff'}),
    dcc.Slider(
        id='input_graph',
        value=[MinT,WindSpeed],
        step=1,
        updatemode='drag',
    ),
])
...
def display_value(value):
    value_x = value[0]
    value_y = value[1]
    data = go.Scatter( x = value_x, y = value_y, name = 'WindSpeed', 
                         mode="markers", opacity=0.8 )
    
    plot_title = "MinT" + " vs. " + "WindSpeed"
    
    layout = dict(title = plot_title, 
                  xaxis=dict(title = 'MinT', ticklen=5, zeroline= False),
                  yaxis=dict(title = 'WindSpeed', side='left'),                                  
                  legend=dict(orientation="h", x=0.4, y=1.0),
                  autosize=False, width=750, height=500,
                 )
